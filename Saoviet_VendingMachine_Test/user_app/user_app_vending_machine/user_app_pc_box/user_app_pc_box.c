...
{
  {_EVENT_PC_BOX_ENTRY,                 0, 5, TIME_ON_DCU,              fevent_pcbox_entry},
  ...
  {_EVENT_PC_BOX_LOG_TSVH,              0, 5, TIME_ENTRY,               fevent_pcbox_log_tsvh},
  ...
  {_EVENT_ON_OFF_CYCLE_PC_BOX,          0, 0, 15000,                    fevent_on_off_cycle_pc_box},
...
/*------------------On Off Cycle Pc Box-----------------*/
    if(sRTC.year > 20)
    {
        if(sCycleOnOffPcBox.State == _ON_CYCLE && sParamPcBox.StatePcBox == _STT_PCBOX_CONNECT && sStatusApp.Delivery == _APP_FREE)
        {
