...
static uint8_t fevent_on_off_sw_pcbox(uint8_t event);
/*=============== Struct =================*/
sEvent_struct               sEventAppPcBox[]=
{
  ...
  {_EVENT_ON_OFF_SW_PCBOX,              0, 0, 0,                        fevent_on_off_sw_pcbox},
};
...
static uint8_t fevent_pcbox_entry(uint8_t event)
{
    ...
    UTIL_Printf_Str(DBLEVEL_L, "ON_MCU\r\n"); 
            ...
            switch(sUartPcBox.Data_a8[pos++])
            {
                ...
                case OBIS_PC_BOX_CTRL_MOTOR:    //Obis Push Motor
                  ...
                  if(Length_Data <= NUMBER_MOTOR * 2 && (pos + Length_Data) <= (sUartPcBox.Length_u16 - 2))
                  {
                    ...
                    {
                        UTIL_Printf_Str(DBLEVEL_L,"Handle: DELIVERY_BUSY\r\n");
                    }
                  }
                  break;
...
//                        pos +=Length_Data;
                ...
                case OBIS_TIME_WARM_RUN:    //Obis set time chay say kinh
                  Length_Data = sUartPcBox.Data_a8[pos++];
                  if(Length_Data == 0x01 && (pos + Length_Data) <= (sUartPcBox.Length_u16 - 2))
                  {
                ...
                case OBIS_TIME_WARM_WAIT:  //Obis set time nghi say kinh
                  Length_Data = sUartPcBox.Data_a8[pos++];
                  if(Length_Data == 0x01 && (pos + Length_Data) <= (sUartPcBox.Length_u16 - 2))
                  {
                ...
                case OBIS_ON_OFF_CYCLE_PC_BOX:  //Obis bat tat che do ON/OFF pc
                  Length_Data = sUartPcBox.Data_a8[pos++];
                  if(Length_Data == 0x01 && (pos + Length_Data) <= (sUartPcBox.Length_u16 - 2))
                  {
                ...
                case OBIS_TIME_OFF_ON_PC_BOX:   //Cai thoi gian tat PC
                  Length_Data = sUartPcBox.Data_a8[pos++];
                ...
                case OBIS_ACCEPT_OFF_PCBOX:     //Cai thoi gian ON/PC
                  Length_Data = sUartPcBox.Data_a8[pos++];
                ...
                case OBIS_SEND_TSVH_INTAN:      //Obis log thong so van hanh tuc thoi
                  Length_Data = sUartPcBox.Data_a8[pos++];
                  if(Length_Data == 0x01 && (pos + Length_Data) <= (sUartPcBox.Length_u16 - 2))
                  {
    ...
    else
    {
        ...
        if(UsingCrc == 0)
        {
            UTIL_Printf_Str(DBLEVEL_L, "PcBox: CRC FAIL\r\n");
        }
    }
    
    if(Recv_Result == 1)
    {
        if(sParamPcBox.StatePcBox != _STT_PCBOX_CONNECT)       //Nhan duoc giu lieu bat ki tu Pc Xac nhan la da ket noi
        {
...
/*--------------------DCU Ping PcBox---------------------*/
  ...
  else
  {
    ...
    fevent_enable(sEventAppPcBox, event);
//    if(sParamPcBox.CountPingPcBox == NUMBER_MAX_PING_PCBOX/2)
//    {
...
/*-----------------Reset PcBox------------------*/
    ...
    if(sParamPcBox.CountResetPcBox <= 1)    //Neu sau 2 lan reset pcbox khong co ket noi thi se ON pcbox o lan thu 3
    {
        if(state_reset == 0)
        {
            ...
            UTIL_Printf_Str(DBLEVEL_L, "app_pc_box: Reset Pc Box\r\n");
...
void  OFF_PcBox(uint32_t TimeOnRelay_ms)
{
    ...
    UTIL_Printf_Str(DBLEVEL_L, "app_pc_box: Sleep PcBox\r\n");
}
...
//    ControlRelay(RELAY_SCREEN, ON_RELAY, _RL_RESPOND, _RL_DEBUG, _RL_CTRL);
    ...
    fevent_enable(sEventAppPcBox, _EVENT_WAIT_RESET_PC_BOX);
    UTIL_Printf_Str(DBLEVEL_L, "app_pc_box: On PcBox\r\n");
}
...
//    sRespPcBox.Data_a8[sRespPcBox.Length_u16++] = sStatusRelay.FridgeCool;
    ...
    sRespPcBox.Data_a8[sRespPcBox.Length_u16++] = 0x00;         //sStatusRelay.FridgeHeat
    sRespPcBox.Data_a8[sRespPcBox.Length_u16++] = 0x00;         //sStatusRelay.FridgeCool
...
//    sRespPcBox.Data_a8[sRespPcBox.Length_u16++] = 0x00;         //sTimeCycleWarm.Run
//    sRespPcBox.Data_a8[sRespPcBox.Length_u16++] = 0x00;         //sTimeCycleWarm.Wait
...
void ResetDCU(void)
{
    ...
    UTIL_Printf_Str(DBLEVEL_L, "OFF_MCU\r\n"); 
//    HAL_Delay(500);
    Reset_Chip();
}
...
uint8_t Save_DcuID(uint8_t *aData, uint16_t Length)
{
    if(Length <= MAX_LENGTH_DCU_ID)
    {
        ...
        Write_Mem_Parameter_Var(EX_FLASH_ADDR_MAIN_ID, write, Length+2);
        return 1;
    }
    return 0;
}
...
uint8_t Save_TimeTSVH(uint8_t Time)
{
    ...
    Write_Mem_Parameter_Var(EX_FLASH_ADDR_FREQ_TSVH, write, 2);
...
{
    uint8_t read[2] = {0};
    Read_Mem_Parameter_Var(EX_FLASH_ADDR_FREQ_TSVH, read, 2);
    
    if( read[0] == BYTE_TEMP_FIRST_EXFLASH)
        sParamPcBox.TimeTSVH  = read[1];
    else
        sParamPcBox.TimeTSVH  = TIME_SEND_TSVH/TIME_ONE_MINUTES;
}
...
uint8_t Save_UsingCrc(uint8_t UsingCrc)
{
    if(UsingCrc <= 1)
    {
        ...
        Write_Mem_Parameter_Var(EX_FLASH_ADDR_PCBOX_USING_CRC, write, 2);
...
{
    uint8_t read[2] = {0};
    Read_Mem_Parameter_Var(EX_FLASH_ADDR_PCBOX_USING_CRC, read, 2);
...
uint8_t Save_CycleOnOffPcBox(uint8_t State, uint8_t HoursOFF, uint8_t MinutesOFF, uint8_t HoursON, uint8_t MinutesON)
{
    ...
    if(State<=1 && HoursOFF<24 && MinutesOFF<60 && HoursON<24 && MinutesON<60)
    {
        ...
        if(ResultCal == 1)
        {
            ...
            Write_Mem_Parameter_Var(EX_FLASH_ADDR_TIME_ON_OFF_PCBOX, write, 6);
...
{
    uint8_t read[6] = {0};
    Read_Mem_Parameter_Var(EX_FLASH_ADDR_TIME_ON_OFF_PCBOX, read, 6);
    if(read[0] == BYTE_TEMP_FIRST_EXFLASH)
    {
        if(read[1]<= 1)
        {
...
uint8_t Save_StatePcBox(uint8_t State)
{
    if(sParamPcBox.StatePcBox != State)
    {
        if(State < _STT_PCBOX_END)
        {
            ...
            Write_Mem_Parameter_Var(EX_FLASH_ADDR_STATE_PCBOX, write, 2);
...
{
    uint8_t read[2] = {0};
    Read_Mem_Parameter_Var(EX_FLASH_ADDR_STATE_PCBOX, read, 2);
    if(read[0] == BYTE_TEMP_FIRST_EXFLASH)
    {
...
//	UTIL_TIMER_Start (&TimerOnOffPcBox);
    ...
    sEventAppPcBox[_EVENT_ON_OFF_SW_PCBOX].e_period = time;
    fevent_enable(sEventAppPcBox, _EVENT_ON_OFF_SW_PCBOX);
}
...
void AppPcBox_Debug(uint8_t aData[], uint8_t length, uint8_t TransRecv)
{
    ...
    {
        UTIL_Printf_Str(DBLEVEL_L, "app_pc_box: Recv: ");
    }
    else
    {
        UTIL_Printf_Str(DBLEVEL_L, "app_pc_box: Trans: ");
    }
    sData sSource=
    {
        .Data_a8 = array,
    ...
    {
        UTIL_Printf_Str(DBLEVEL_L, "Data Large");
    }
