...
void        CallBack_IRQ_HAL_GetTick_Ms(void)
{
    sTimeLockMagnetis.GetTick_Ms++;
}

uint8_t Write_Mem_Parameter_Var(uint32_t SectorAddr, uint8_t *Data_u8, uint16_t Length)
{
    char str[100];
    
    if(eFlash_S25FL_Erase_Sector(SectorAddr) == 0)
    {
        sprintf(str, "Mem_Earse: Fail Addr = 0x%08lX\r\n", SectorAddr);

        UTIL_Printf_Str(DBLEVEL_M, str);
        return 0;
    }
    
    if(eFlash_S25FL_BufferWrite(Data_u8, SectorAddr, Length) == 0)
    {
        sprintf(str, "Mem_Write: Fail Addr = 0x%08lX\r\n", SectorAddr);

        UTIL_Printf_Str(DBLEVEL_M, str);
        return 0;
    }
    
    sprintf(str, "Mem_Write: Success Addr = 0x%08lX\r\n", SectorAddr);
...
uint8_t Read_Mem_Parameter_Var(uint32_t SectorAddr, uint8_t *Data_u8, uint16_t Length)
{
    char str[100];

    if(eFlash_S25FL_BufferRead(Data_u8, SectorAddr , Length) == 0)
    {
        sprintf(str, "Mem_Read: Fail Addr = 0x%08lX\r\n", SectorAddr);
    ...
    sprintf(str, "Mem_Read: Success Addr = 0x%08lX\r\n", SectorAddr);
