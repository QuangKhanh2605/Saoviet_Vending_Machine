...
static uint8_t fevent_protect_motor_elevator(uint8_t event);
...
static uint8_t fevent_free_lock_magnetis(uint8_t event);
...
{   
  ...
  { _EVENT_HANDLE_ERROR_DELIVERY,     0, 5, 0,                          fevent_handle_error_delivery},
  ...
  { _EVENT_FREE_ELEVATOR_LOWER,       1, 5, 0,                          fevent_free_elevator_lower},
  
  { _EVENT_FIX_MOTOR,                 0, 0, 0,                          fevent_fix_motor},
  
  { _EVENT_PROTECT_MOTOR_ELEVATOR,    1, 0, 50,                         fevent_protect_motor_elevator},
  ...
  { _EVENT_FREE_LOCK_MAGNETIS,        1, 5, 500,                        fevent_free_lock_magnetis},  //default 2000ms
};
...
uint32_t Isense_MotorElevator = 0;

Struct_TimeLockMagnetis     sTimeLockMagnetis = {0};
/*================ Function Handler =================*/
static uint8_t fevent_motor_entry(uint8_t event)
{   
    ...
    //Luon mo khoa tu
    Control_LockMagnetis(UNLOCK_MAGNETIS);     // UnLock LockMagnetis
...
static uint8_t fevent_handle_error_delivery(uint8_t event)
{
    ...
    switch(DetectErrorDelivery)
    {
        case _EVENT_LOCK_MAGNETIS_BEGIN:
            UTIL_Printf_Str (DBLEVEL_M, "Delivery Error: _EVENT_LOCK_MAGNETIS_BEGIN\r\n");
          break;
          
        case _EVENT_ELEVATOR_LOWER_BEGIN:
            UTIL_Printf_Str (DBLEVEL_M, "Delivery Error: _EVENT_ELEVATOR_LOWER_BEGIN\r\n");
          break;
          
        case _EVENT_DELIVERY_ENTRY:
            UTIL_Printf_Str (DBLEVEL_M, "Delivery Error: _EVENT_DELIVERY_ENTRY\r\n");
          break;
          
        case _EVENT_CALCULATOR_COORDINATES:
            UTIL_Printf_Str (DBLEVEL_M, "Delivery Error: _EVENT_CALCULATOR_COORDINATES\r\n");
          break;
          
        case _EVENT_PUSH_MOTOR:
            UTIL_Printf_Str (DBLEVEL_M, "Delivery Error: _EVENT_PUSH_MOTOR\r\n");
          break;
          
        case _EVENT_ELEVATOR_LOWER_END:
            UTIL_Printf_Str (DBLEVEL_M, "Delivery Error: _EVENT_ELEVATOR_LOWER_END\r\n");
          break;
          
        case _EVENT_LOCK_MAGNETIS_END:
            UTIL_Printf_Str (DBLEVEL_M, "Delivery Error: _EVENT_LOCK_MAGNETIS_END\r\n");
          break;
          
        case _EVENT_FINISH_DELIVERY:
            UTIL_Printf_Str (DBLEVEL_M, "Delivery Error: _EVENT_FINISH_DELIVERY\r\n");
          break;
...
//        return 1;
    ...
    Control_LockMagnetis(LOCK_MAGNETIS);     // Lock LockMagnetis
...
//        
//        Enable_Handle_Idle_Delivery(_EVENT_PUSH_MOTOR, ((sParamDelivery.aDataPush[sParamDelivery.PosHandle - 1])*TIME_MOTOR_PUSH_LATE + 3000)); //Enable handle error
//  
//        return 1;
...
static uint8_t fevent_push_motor(uint8_t event)
{
    ...
    if(sMotorPush.aPulse[sParamDelivery.PosHandle - 1] < sParamDelivery.aDataPush[sParamDelivery.PosHandle - 1])
    {
        ...
        else if(HAL_GetTick() - sMotorPush.TimeOnMotor >= TIME_MOTOR_PUSH_LATE)
        {
            ...
            UTIL_Printf_Str(DBLEVEL_M, "Push_Late\r\n");
        }
...
static uint8_t fevent_finish_delivery(uint8_t event)
{
    ...
    if(sParamDelivery.StateHanlde == DELIVERY_PURCHASE)
        ...
        Respond_Delivery_PcBox(RESPOND_FIX_MOTOR);  //Phan hoi lai PcBox
    
    Delivery_Handle_State(DELIVERY_FREE);
    
    Disable_Handle_Idle_Delivery(); //Disable handle error
    UTIL_Printf_Str(DBLEVEL_M, "Purchase Success\r\n");
    return 1;
}
...
static uint8_t fevent_free_elevator_lower(uint8_t event)
{
    ...
    sEventAppDelivery[_EVENT_FREE_ELEVATOR_LOWER].e_period = 1000;
    fevent_enable(sEventAppDelivery, event);
    return 1;
}
...
//        On_Motor_Push(sParamDelivery.PosHandle);
    ...
    else
    {
        ...
        Delivery_Handle_State(DELIVERY_FREE);
        Off_Motor_Push();
        UTIL_Printf_Str(DBLEVEL_M, "FIX_MOTOR Success\r\n");
        Handle_Once = 0;
        return 1;
    }
    ...
    if(HAL_GetTick() - gettick_ms_error >= TIME_MOTOR_PUSH_LATE)
    {
        ...
        Off_Motor_Push();
        Delivery_Handle_State(DELIVERY_FREE);
        UTIL_Printf_Str(DBLEVEL_M, "FIX_MOTOR error\r\n");
        Handle_Once = 0;
        return 1;
    }
...
static uint8_t fevent_protect_motor_elevator(uint8_t event)
{
    ...
    if(Isense_MotorElevator > MAX_CURRENT_MOTOR_ELEVATOR)
        count_error++;
    else
        count_error = 0;
    ...
    if(count_error > 4)
    {
        ...
        if(sParamDelivery.StateHanlde == DELIVERY_FREE)
        {
            sEventAppDelivery[_EVENT_FREE_ELEVATOR_LOWER].e_period = 5*60000;
            fevent_enable(sEventAppDelivery,_EVENT_FREE_ELEVATOR_LOWER);
        }
    }
...
static uint8_t fevent_test_elevator(uint8_t event)
{
    ...
    if(StateHandle == 1)
    {
        switch(Pos_Elevator)
        {
            ...
            case _ELEVATOR_BOT:
                Delivery_Handle_State(DELIVERY_FREE);
                UTIL_Printf_Str(DBLEVEL_M, "Elevator Success\r\n");
                Handle_Once = 0;
                StateHandle = 0;
                return 1;
    ...
    {
        Control_Elevator(ELEVATOR_STOP, LEVEL_PWM_ELEVATOR_OFF);
        UTIL_Printf_Str(DBLEVEL_M, "Elevator Error\r\n");
        Delivery_Handle_State(DELIVERY_FREE);
        Handle_Once = 0;
        StateHandle = 0;
        return 1;
...
static uint8_t fevent_free_lock_magnetis(uint8_t event)
{
    ...
    static uint32_t gettick_ms2 = 0;
    static uint8_t  state_before = DELIVERY_FREE;
    uint8_t Pos_Elevator = 0;
    
    if(sParamDelivery.StateHanlde != DELIVERY_FREE)
    {
        sTimeLockMagnetis.Time_Ctrl = sTimeLockMagnetis.GetTick_Ms;
    }
    else
    {
        if(sTimeLockMagnetis.GetTick_Ms - sTimeLockMagnetis.Time_Ctrl > 5*60000)
        {
            Control_LockMagnetis(LOCK_MAGNETIS);     // Lock LockMagnetis
            if(sParamDelivery.LockMagnetis == LOCK_MAGNETIS_OPEN)
            {
                if(HAL_GetTick() - gettick_ms > 60000)
                {
                    gettick_ms = HAL_GetTick();
                    sRespPcBox.Length_u16 = 0;
                    sRespPcBox.Data_a8[sRespPcBox.Length_u16++] = OBIS_REQUEST_CLOSE_LOCK_MAGNETIS;
    ...
    if(HAL_GetTick() - gettick_ms2 > 30000)
    {
        gettick_ms2 = HAL_GetTick();
        
        if(sTimeLockMagnetis.GetTick_Ms - sTimeLockMagnetis.Time_Ctrl < 6*60000)
        {
            Save_TimeLockMagnetis();
        }
    }
    
    if(state_before != sParamDelivery.StateHanlde)
    {
        state_before = sParamDelivery.StateHanlde;
        Save_TimeLockMagnetis();
    }
...
//    return 0;
    ...
    if(sParamDelivery.StateHanlde == DELIVERY_FREE)
    {
        ...
        sParamDelivery.aDataPush[Pos - 1] = 1;
...
void Delivery_Entry(void)
{
    ...
    if(sParamDelivery.SumDelivery > 0)
    {
        ...
        fevent_active(sEventAppDelivery, _EVENT_LOCK_MAGNETIS_BEGIN);
        Enable_Handle_Idle_Delivery(_EVENT_LOCK_MAGNETIS_BEGIN, 5000);
...
void IRQ_Pluse_OffMotorPush(uint8_t Pos)
{
    ...
    {
        if(HAL_GetTick() - sMotorPush.TimeOnMotor < TIME_MOTOR_PUSH_EARLY)
        {
           UTIL_Printf_Str(DBLEVEL_M, "Push_Early\r\n");
        }
        else 
        {
...
void Off_Motor_Push(void)
{
    if(sMotorPush.State == MOTOR_PUSH_ON)
    {
        ...
        char aData[50]={0};
        sprintf(aData, "app_ctrl_motor: TimePush: %d\r\n", sMotorPush.TimeOnMotor);
...
void Respond_Delivery_PcBox(eStateRespondPcBox StateRes)
{
    ...
    switch(StateRes)
    {
        ...
        case RESPOND_WARNING_DELIVERY:
            ...
            sRespPcBox.Data_a8[sRespPcBox.Length_u16++] = 0x01;
            if(DetectErrorDelivery == _EVENT_LOCK_MAGNETIS_BEGIN)
                sRespPcBox.Data_a8[sRespPcBox.Length_u16++] = 0x02;
            else 
                sRespPcBox.Data_a8[sRespPcBox.Length_u16++] = 0x01;
          break;  
...
void AppDelivery_Debug(uint8_t KindDelivery)
{
    ...
    {
        case RESPOND_PUSH_MOTOR:
          sprintf(aData, "app_ctrl_motor: Push:%d | Number:%d\r\n", sParamDelivery.PosHandle, sParamDelivery.aDataPush[sParamDelivery.PosHandle-1]);
          UTIL_Printf_Str(DBLEVEL_M, aData);
          break;
          
        case RESPOND_FIX_MOTOR:
          sprintf(aData, "app_ctrl_motor: FixMotor:%d\r\n", sParamDelivery.PosHandle);
          UTIL_Printf_Str(DBLEVEL_M, aData);
          break;
          
        case RESPOND_COMPLETE_DELIVERY:
          sprintf(aData, "app_ctrl_motor: Weight:%d\r\n", sParamDelivery.Weighing);
          UTIL_Printf_Str(DBLEVEL_M, aData);
          break;
...
uint8_t Check_Top_Bot_Elevator(void)
{
    ...
    else if(state_top == 1 && state_bot == 0)
    {
    ...
    else if(state_top == 0 && state_bot == 1)
    {
...
{
    char aData[100]={0};
    sprintf(aData, "Encoder_C1: %d | Encoder_C2: %d\r\n", sElevator.Encoder_C1, sElevator.Encoder_C2);
    UTIL_Printf_Str(DBLEVEL_M, aData);
}
    ...
    if(State != State_Before)
    {
        ...
        switch(State)
        {
            case DELIVERY_FREE:
              ...
              UTIL_Printf_Str(DBLEVEL_M, "Hanlde: DELIVERY_FREE\r\n");
              fevent_active(sEventAppDelivery,_EVENT_PROTECT_MOTOR_ELEVATOR);
              break;
              
            case DELIVERY_PURCHASE:
              sParamDelivery.StateHanlde = DELIVERY_PURCHASE;
              UTIL_Printf_Str(DBLEVEL_M, "Hanlde: DELIVERY_PURCHASE\r\n");
              break;
              
            case DELIVERY_TEST_ELEVATOR:
              sParamDelivery.StateHanlde = DELIVERY_TEST_ELEVATOR;
              UTIL_Printf_Str(DBLEVEL_M, "Hanlde: DELIVERY_TEST_ELEVATOR\r\n");
              break;
              
            case DELIVERY_FIX_MOTOR:
              sParamDelivery.StateHanlde = DELIVERY_FIX_MOTOR;
              UTIL_Printf_Str(DBLEVEL_M, "Hanlde: DELIVERY_FIX_MOTOR\r\n");
              break;
...
/*---------------------Save and Init Time Lock Magnetis-----------------*/
void Save_TimeLockMagnetis(void)
{
    uint8_t write[12]={0x00};
    
    write[0] = BYTE_TEMP_FIRST_EXFLASH;
    write[1] = sTimeLockMagnetis.GetTick_Ms >> 24;
    write[2] = sTimeLockMagnetis.GetTick_Ms >> 16;
    write[3] = sTimeLockMagnetis.GetTick_Ms >> 8;
    write[4] = sTimeLockMagnetis.GetTick_Ms;
    
    write[5] = sTimeLockMagnetis.Time_Ctrl >> 24;
    write[6] = sTimeLockMagnetis.Time_Ctrl >> 16;
    write[7] = sTimeLockMagnetis.Time_Ctrl >> 8;
    write[8] = sTimeLockMagnetis.Time_Ctrl;
    
    Write_Mem_Parameter_Var(EX_FLASH_ADDR_TIME_LOCK_MAGNETIS, write, 12);
}

void Init_TimeLockMagnetis(void)
{
    uint8_t read[10] = {0};
    Read_Mem_Parameter_Var(EX_FLASH_ADDR_TIME_LOCK_MAGNETIS, read, 12);
    if( read[0] == BYTE_TEMP_FIRST_EXFLASH)
    {
        sTimeLockMagnetis.GetTick_Ms = read[1];
        sTimeLockMagnetis.GetTick_Ms = (sTimeLockMagnetis.GetTick_Ms << 8) | read[2];
        sTimeLockMagnetis.GetTick_Ms = (sTimeLockMagnetis.GetTick_Ms << 8) | read[3];
        sTimeLockMagnetis.GetTick_Ms = (sTimeLockMagnetis.GetTick_Ms << 8) | read[4];
        
        sTimeLockMagnetis.Time_Ctrl = read[5];
        sTimeLockMagnetis.Time_Ctrl = (sTimeLockMagnetis.Time_Ctrl << 8) | read[6];
        sTimeLockMagnetis.Time_Ctrl = (sTimeLockMagnetis.Time_Ctrl << 8) | read[7];
        sTimeLockMagnetis.Time_Ctrl = (sTimeLockMagnetis.Time_Ctrl << 8) | read[8];
    }
    else
    {
        sTimeLockMagnetis.GetTick_Ms = 0;
        sTimeLockMagnetis.Time_Ctrl  = 0;
    }
}
