...
//			id ENUM				  kind process			     str Received
        ...
        {_GET_V_SENSE_ELEVATOR, _fGET_V_SENSE_ELEVATOR,     {(uint8_t*)"at+motorele?",11}},
            ...
            else
            {
                switch(sParamPcBox.StatePcBox)
                {
                    case _STT_PCBOX_CONNECT:
                      UTIL_Printf_Str(DBLEVEL_L, "PcBox CONNECT\r\n");
                      break;
                      
                    case _STT_PCBOX_DISCONNECT:
                      UTIL_Printf_Str(DBLEVEL_L, "PcBox DISCONNECT\r\n");
                      break;
            ...
            else
            {
                switch(sParamPcBox.StatePcBox)
                {
                    ...
                    case _STT_PCBOX_SLEEP:
                      ...
                      UTIL_Printf_Str(DBLEVEL_L, "PcBox SLEEP");
                      break;
...
void        _fGET_CURRENT_WEIGHT (sData *strRecei, uint16_t Pos)
{
    ...
    {
        char aData[50]={0};
        sprintf(aData, "Weight:  %d\r\n", sParamDelivery.Weighing);
        UTIL_Printf_Str(DBLEVEL_M, aData);
    }
    else
        UTIL_Printf_Str(DBLEVEL_M, "Weight_Disconnect!!!");
...
void        _fTEST_LOCK_MAGNETIS (sData *strRecei, uint16_t Pos)
{
    ...
    if( strRecei->Data_a8[0] >= '0' && strRecei->Data_a8[0] <= '9')
    {
        ...
        {
            Control_LockMagnetis(LOCK_MAGNETIS);
            DCU_Respond(PortConfig, (uint8_t*)"OK\r\n", 4, 0);
        }
        else 
        {
...
void        _fCHECK_TOP_ELEVATOR (sData *strRecei, uint16_t Pos)
{
    if(HAL_GPIO_ReadPin(FB_Top_Elevator_GPIO_Port, FB_Top_Elevator_Pin) == GPIO_PIN_SET)
        UTIL_Printf_Str(DBLEVEL_M, "TOP_ELEVATOR: ON\r\n");
    else
        UTIL_Printf_Str(DBLEVEL_M, "TOP_ELEVATOR: OFF\r\n");
}
...
void        _fCHECK_BOT_ELEVATOR (sData *strRecei, uint16_t Pos)
{
    if(HAL_GPIO_ReadPin(FB_Bot_Elevator_GPIO_Port, FB_Bot_Elevator_Pin) == GPIO_PIN_SET)
        UTIL_Printf_Str(DBLEVEL_M, "BOT_ELEVATOR: ON\r\n");
    else
        UTIL_Printf_Str(DBLEVEL_M, "BOT_ELEVATOR: OFF\r\n");
}

void        _fCHECK_LOCK_MAGNETIS (sData *strRecei, uint16_t Pos)
{
    if(sParamDelivery.LockMagnetis == LOCK_MAGNETIS_OPEN)
        UTIL_Printf_Str(DBLEVEL_M, "LOCK_MAGNETIS: OPEN\r\n");
    else
        UTIL_Printf_Str(DBLEVEL_M, "LOCK_MAGNETIS: CLOSE\r\n");
}
...
void        _fGET_V_SENSE_ELEVATOR (sData *strRecei, uint16_t Pos)
{
    uint8_t aDataRespond[50]={0};
    uint16_t lengthRespond = 0;
    
    Insert_String_To_String(aDataRespond, &lengthRespond, (uint8_t*)"Isense: ",0 , 8);
    Convert_Point_Int_To_String_Scale (aDataRespond, &lengthRespond, (int)(Isense_MotorElevator), 0x00);
    Insert_String_To_String(aDataRespond, &lengthRespond, (uint8_t*)" mA",0 , 3);
    Insert_String_To_String(aDataRespond, &lengthRespond, (uint8_t*) "\r\n",0 , 2);
