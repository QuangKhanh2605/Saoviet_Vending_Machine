...
static uint8_t fevent_test_ele_use_variable(uint8_t event);
...
{   
  ...
  { _EVENT_TEST_ELE_USE_VARIABLE,     1, 0, 0,                          fevent_test_ele_use_variable},
};
...
static uint8_t fevent_test_elevator(uint8_t event)
{
    ...
    if(HAL_GetTick() - gettick_ms_error > 20000)
    {
        OFF_Elevator();
...
uint8_t Check_Top_Bot_Elevator(void)
{
    ...
    if(HAL_GPIO_ReadPin(FB_Top_Elevator_GPIO_Port, FB_Top_Elevator_Pin) == GPIO_PIN_RESET)
      state_top = 1;
    
    if(HAL_GPIO_ReadPin(FB_Bot_Elevator_GPIO_Port, FB_Bot_Elevator_Pin) == GPIO_PIN_RESET)
      state_bot = 1;
