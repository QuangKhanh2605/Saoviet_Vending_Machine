...
void eFlash_Get_Infor (uint32_t AddStart, uint8_t *pData, uint16_t *Length, uint8_t MAX_LEGNTH_INFOR)
{
    ...
    eFlash_S25FL_BufferRead(aData, AddStart , 1);

    if (aData[0] > MAX_LEGNTH_INFOR)
        aData[0] = MAX_LEGNTH_INFOR;
    //clear buff
    UTIL_MEM_set (pData, 0, MAX_LEGNTH_INFOR);
    //Read flash
    eFlash_S25FL_BufferRead(pData, (AddStart + 1) , aData[0]);
    *Length = aData[0];
}
...
uint8_t Save_Array_Ex_Flash(uint32_t ADD, uint8_t* Buff, uint16_t length)
{
    ...
    eFlash_S25FL_Erase_Sector(ADD);
    

    aTemp[0] = BYTE_TEMP_FIRST_EXFLASH;
    aTemp[1] = length;
    for(i = 0; i < length; i++)
        aTemp[i+2] = *(Buff + i);
    ...
    if(eFlash_S25FL_BufferWrite(aTemp, ADD, length) != HAL_OK)
        return 0;
